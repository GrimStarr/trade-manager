...
const Dashboard = () => {
  ...
  useEffect(
    () =>
      onSnapshot(serverCollection, (snapshot: QuerySnapshot<DocumentData>) => {
        setServers(
  ...
  const fetchToServer = async (value: any) => {
    ...
    if (value.status) {
      axios
        .post(`http://${value.ip}:5009/buy`, data)
        .then((response) => {
          //   response.data.map((value: string) => toast.success(value));
  ...
  const sendOrder = async (e: any) => {
    ...
    servers.map((server) => fetchToServer(server));
  };
      ...
      <div className="container mx-auto ">
        <div className="flex flex-col lg:flex-row w-full lg:w-8/12 bg-white rounded-xl mx-auto shadow-lg overflow-hidden">
          <div className="w-full lg:w-1/2 flex flex-col items-center justify-center p-4 bg-no-repeat bg-center bg-cover bg-picture">
            <h1
              className="text-white font-medium text-3xl "
              onClick={() => {
                router.push("/servers");
              }}
            >
          ...
          <div className="w-full lg:w-1/2 py-2 px-12">
            <form onSubmit={sendOrder}>
              ...
              <div className="mt-5">
                <input
                  onChange={(e: any) =>
                    setData({ ...data, sl: e.target.value })
                  }
                  type="decimal"
                  inputMode="decimal"
              ...
              <div className="mt-5">
                <input
                  onChange={(e: any) =>
                    setData({ ...data, tp1: e.target.value })
                  }
                  value={data.tp1}
                  type="decimal"
                  inputMode="decimal"
              ...
              <div className="mt-5">
                <input
                  onChange={(e: any) =>
                    setData({ ...data, tp2: e.target.value })
                  }
                  value={data.tp2}
                  type="decimal"
                  inputMode="decimal"
              ...
              <div className="mt-5">
                <input
                  onChange={(e: any) =>
                    setData({ ...data, tp3: e.target.value })
                  }
                  value={data.tp3}
                  type="decimal"
                  inputMode="decimal"
              ...
              <div className="mt-5 grid grid-cols-2 gap-1.5">
                <button
                  onClick={() => setData({ ...data, type: "BUY" })}
                  className="w-full bg-green-500 py-3 text-center text-white font-medium rounded-md shadow-md transition hover:bg-green-600"
                >
                  Buy
                </button>
                <button
                  onClick={() => setData({ ...data, type: "SELL" })}
                  className="w-full bg-red-600 py-3 text-center text-white font-medium rounded-md shadow-md transition hover:bg-red-700"
                >
                  Sell
                </button>
              </div>
